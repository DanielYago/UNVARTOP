%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
% A 210 line topology optimization code using the Unsmooth Variational Topology method %
%   for compliant mechanism topology optimization problems                             %
%   Paper's Title                                                                      %
...
% Versions                                                                             %
%   26/3/2020 - Submission version (compliant mechanism)                               %
% GitHub repository																	   %
...
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
function [iter,J] = UNVARTOP_2D_complmechanism (nelx,nely,nsteps,Vol0,Vol,k,tau)
...
F(n_unkn*find(coord(:,2)>=0.9*nely & coord(:,1)==nelx)-1,2) = -0.0001*nelx;
fixed_dofs = [reshape(n_unkn*find(coord(:,2)==nely),1,[]),...
	reshape(n_unkn*find(coord(:,1)==0 & coord(:,2)<=0.1*nely)+(-n_unkn+1:0),1,[])];
active_node = find(coord(:,2)>0.9*nely&(coord(:,1)<0.05*nelx|coord(:,1)>0.95*nelx));
passive_node = [];
...
m = 3; E0 = 1; alpha = 1e-2; beta = nthroot(alpha,m); nu = 0.3;
%% Prepare animation
psi_vec = zeros(size(coord,1),nsteps+1);
chi_vec = zeros(size(connect,1),nsteps+1);
U_vec = zeros(n_unkn*size(coord,1),2,nsteps+1);
...
for i_step = 1:nsteps
	...
	while (((Tol_chi>1e-1 || Tol_lambda>1e-1) && iter_step<iter_max_step) || iter_step<=iter_min_step)
		% FE-analysis
		[K] = assmebly_stiff_mat (chi,KE,KE_cut,beta,m,iK,jK,n_unkn,nelx,nely,id_in,id_out);
		U(free_dofs,:) = K(free_dofs,free_dofs) \ (F(free_dofs,:) - K(free_dofs,fixed_dofs)*U(fixed_dofs,:));
		if iter == 1; U_vec(:,:,1)=U; J_ref = -abs(F(:,2)'*U(:,1)); end; J = F(:,2)'*U(:,1)/J_ref;
		% Calculate sensitivities
		Energy = zeros(ngaus,nelx*nely);
		id = chi==1|chi==0; int_chi = interp_property (m,m-1,beta,chi(id));
		u_e = reshape(U(edofMat(id,:)',1),n_nodes*n_unkn,[]);
		w_e = -reshape(U(edofMat(id,:)',2),n_nodes*n_unkn,[]);
		for i=1:ngaus; Energy(i,id) = sum(w_e.*(KE_i(:,:,i)*u_e),1); end
		Energy(:,id) = int_chi.*Energy(:,id);
		id = ~id; int_chi = interp_property (m,m-1,beta,chi(id));
		u_e = reshape(U(edofMat(id,:)',1),n_nodes*n_unkn,[]);
		w_e = -reshape(U(edofMat(id,:)',2),n_nodes*n_unkn,[]);
		Energy(:,id) = repmat(int_chi.*sum(w_e.*(K_cut*u_e),1),ngaus,1);
	...
	if iter > iter_max; warning('UNVARTOP_2D_complmechanism:Max_iter','Maximum number of iterations achieved.'); break; end
end
%% Animation
...
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
function [K] = assmebly_stiff_mat (chi,KE,KE_cut,beta,m,iK,jK,n_unkn,nelx,nely,id_in,id_out)
