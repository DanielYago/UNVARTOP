...
%   for structural problems - 4.5 Gripper (compliant mechanism)                        %
%   Paper's Title																	   %
...
case_type='MECHANISM'; load_states=2;
...
fixed_dofs = [reshape(n_unkn*find(coord(:,2)==nely),1,[]),reshape(n_unkn*find(coord(:,1)==0&coord(:,2)<=0.1*nely)+(-n_unkn+1:0),1,[])];
active_node = [find(coord(:,2)>0.9*nely&coord(:,1)<0.05*nelx);find(coord(:,2)>0.9*nely&coord(:,2)<=0.95*nely&coord(:,1)>=0.9*nelx)];
passive_node = [find(coord(:,1)>0.8*nelx&coord(:,1)<0.9*nelx&coord(:,2)>0.8*nely);find(coord(:,1)>=0.9*nelx&coord(:,2)>0.95*nely)];
...
if strcmp(case_type,'MECHANISM')
	K(id_in)  = K(id_in)  + 0.01;
	K(id_out) = K(id_out) + 0.01;
end
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
