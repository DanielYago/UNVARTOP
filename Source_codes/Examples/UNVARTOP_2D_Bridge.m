...
%   for structural problems - 4.4 Bridge                                               %
%   Paper's Title																	   %
...
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
function [iter,J] = UNVARTOP_2D_Bridge (nelx,nely,nsteps,Vol0,Vol,k,tau)
...
F(n_unkn*find(coord(:,2)==floor(nely*1.6/5)),1)= -0.01*nelx;
fixed_dofs = [reshape(n_unkn*find(coord(:,1)==0)-1,1,[]),...
              reshape(n_unkn*find(coord(:,1)>=5.75/6*nelx&coord(:,2)==0)+(-n_unkn+1:0),1,[]),...
              reshape(n_unkn*find(coord(:,1)==nelx&coord(:,2)==floor(nely*1.5/5)),1,[])];
active_node = find(coord(:,2)>=nely*1.5/5&coord(:,2)<=nely*1.6/5); passive_node = [];
