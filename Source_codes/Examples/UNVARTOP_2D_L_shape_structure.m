...
%   for structural problems - 4.3 L-shape structure                                    %
%   Paper's Title																	   %
...
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
function [iter,J] = UNVARTOP_2D_L_shape_structure (nelx,nely,nsteps,Vol0,Vol,k,tau)
...
F(n_unkn*find(coord(:,2)==round(0.2*nely)&coord(:,1)==nelx),1) = -0.01*nelx;
fixed_dofs = reshape(n_unkn*find(coord(:,1)<=0.4*nelx&coord(:,2)==nely)+(-n_unkn+1:0),1,[]);
active_node = []; passive_node = find(coord(:,1)>ceil(nelx*0.4)&coord(:,2)>ceil(nely*0.4));
