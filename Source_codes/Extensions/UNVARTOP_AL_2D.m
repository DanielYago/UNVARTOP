%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
% A 215 line topology optimization code using the Unsmooth Variational Topology method      %
...
% Versions                                                                                  %
%   26/3/2020 - Submission version (minimum compliance using Augmented Lagrangian method)   %
% GitHub repository																	   		%
...
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
function [iter,J] = UNVARTOP_AL_2D (nelx,nely,nsteps,Vol0,Vol,k,tau,rho0)
n_dim = 2; n_unkn = 2; n_nodes = 4; ngaus = 4; n = (nelx+1)*(nely+1); h_e = 1; alpha0 = 1e-3;
iter_max_step = 500; iter_min_step = 4; iter_max = 5000;
...
case_type='COMPLIANCE'; load_states=1;
...
[vol,chi] = compute_volume (psi,connect); chi0_step = chi; chi_vec(:,1) = chi';
...
for i_step = 1:nsteps
	...
	while (((Tol_chi>1e-1 || abs(Tol_constr)>1e-3 || Tol_lambda>1e-1) && iter_step<iter_max_step) || iter_step<=iter_min_step )
		% FE-analysis
		...
		J = J + nelx*nely*(lambda*Tol_constr + rho*Tol_constr^2)/J_ref*xi_norm;
		% Compute topology
		[lambda,rho,chi_n,psi,vol,Tol_constr] = find_volume (iter,xi,connect,active_node,passive_node,t_ref,lambda,rho,rho0,alpha0,Tol_constr,Tol_constr_vec);
		...
		chi = chi_n;
		Tol_constr_vec = [Tol_constr_vec,abs(Tol_constr)];
	...
	if iter_step >= iter_max_step; warning('UNVARTOP_AL_2D:Max_iter_step','Maximum number of in-step iterations achieved.'); break; end
	if iter > iter_max; warning('UNVARTOP_AL_2D:Max_iter','Maximum number of iterations achieved.'); break; end
end
%% Animation
...
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
function [lambda,rho,chi,psi,vol,Tol_constr] = find_volume (iter,xi,connect,active_node,passive_node,t_ref,lambda,rho,rho0,alpha0,Tol_constr,Tol_constr_vec)
...
if iter>=3; rho = min(0.02*rho*(abs(diff(Tol_constr_vec(end-1:end)))<1e-3) + rho,100*rho0); end
%%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%% %%%%
